...
function sql(strings, ...values) {
	const out = [];
	let i = 0;
	for (; i < values.length; ++i) out.push(strings[i], escapeValue(values[i]).toSqlString());
	out.push(strings[i]);
	return frag(out.join(""));
}
/** @internal */
function escapeValueRaw(value) {
	if (isFrag(value)) return value.toSqlString();
	...
	if (value instanceof Date) return `TIMESTAMP'${value.toISOString().replace("T", " ").replace(/(?:\.000)?Z$/, "")}'`;
...
function getFields(o) {
	return Object.keys(o).filter((k) => o[k] !== void 0);
}
(function(_sql) {
	function set(fields) {
		if (Array.isArray(fields)) {
			const filteredFields = fields.filter((p) => p[1] !== void 0);
			if (!filteredFields.length) throw new Error("No fields defined");
			return frag(filteredFields.map((f) => `${escapeIdStrictRaw(f[0])}=${escapeValueRaw(f[1])}`).join(", "));
		}
		const filteredFields = getFields(fields);
		if (!filteredFields.length) throw new Error("No fields defined");
		return frag(filteredFields.map((fieldName) => `${escapeIdLooseRaw(fieldName)}=${escapeValueRaw(fields[fieldName])}`).join(", "));
	}
	_sql.set = set;
	function insert(table, data, options = EMPTY_OBJECT) {
		...
		if (options.onDuplicateKey === "ignore") {
			let firstCol;
			if (Array.isArray(data)) firstCol = data[0][0];
			else firstCol = Object.keys(data)[0];
			const escCol = frag(escapeIdLooseRaw(firstCol));
			q = sql`${q} ON DUPLICATE KEY UPDATE ${escCol}=${escCol}`;
		} else if (options.onDuplicateKey === "update") {
			let cols;
			if (Array.isArray(data)) cols = data.map((f) => f[0]);
			else cols = getFields(data);
	...
	function alias(fields) {
		if (Array.isArray(fields)) return frag(fields.map((f) => `${escapeIdStrictRaw(f[0])} AS ${escapeIdStrictRaw(f[1])}`).join(", "));
		return frag(getFields(fields).map((alias) => `${escapeIdStrictRaw(fields[alias])} AS ${escapeIdStrictRaw(alias)}`).join(", "));
	}
	_sql.alias = alias;
...
var PoolConnection = class {
	...
	async *stream(query) {
		...
		let done = false;
		this.conn.queryStream(query.toSqlString()).on("error", (err) => {
			throw err;
		}).on("data", (row) => {
			results.push(row);
			resolve();
...
var ConnectionPool = class {
	...
	async transaction(callback) {
		if (Array.isArray(callback)) return this.transaction(async (conn) => {
...
//#endregion
export { ConnectionPool, DuplicateKey, createPool, escapeIdLooseRaw as escapeIdLoose, escapeIdStrictRaw as escapeIdStrict, escapeValueRaw as escapeValue, sql };
